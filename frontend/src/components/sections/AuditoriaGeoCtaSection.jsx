    ...
    <section style={{ padding: '40px 0 60px' }}>
      <div className="cta-band">
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          <span className="eyebrow centered">{t('auditoria.geoCta.eyebrow')}</span>
          <h2 className="display" style={{ marginTop: 18, maxWidth: 720, marginInline: 'auto' }}>
            {t('auditoria.geoCta.h2')}
          </h2>
          <p style={{ color: 'var(--fg-dim)', fontSize: 17, maxWidth: 560, margin: '22px auto 0' }}>
            {t('auditoria.geoCta.text')}
          </p>
          <div style={{ marginTop: 32, display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
            <a className="btn-pill" href={`${prefix}/contacto/`}>{t('auditoria.geoCta.cta')} <ArrowUpRight /></a>
          </div>
        </div>
      </div>
    </section>
